...
Latter House Life Planner is a **pnpm monorepo**. The runnable app is the Express API (`@workspace/api-server`) plus the Vite React SPA (`@workspace/latter-house`). In production the API serves the built SPA (single server).

### Environment
- Use **pnpm** (the root `preinstall` rejects npm/yarn). The startup update script runs `pnpm install`.
- Node 22 works fine.
- A local **PostgreSQL 16** is required. Start it with `sudo pg_ctlcluster 16 main start`. Convention used for local dev: database `lhl_planner`, role `dev`/`dev`, i.e. `DATABASE_URL=postgresql://dev:dev@127.0.0.1:5432/lhl_planner`.
